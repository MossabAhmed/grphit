...
function fillProductSelects() { [['importItem','اختر الصنف'], ['exportItem','اختر الصنف']].forEach(([id, placeholder]) => { const select = $(id); const selected = select.value; select.innerHTML = `<option value="">${placeholder}</option>` + state.products.map((p) => `<option value="${esc(p.id)}">${esc(p.name)} (${esc(unitLabel(p.unitCategory, p.baseUnit))})</option>`).join(''); if (state.products.some((p) => p.id === selected)) select.value = selected; }); populateMovementUnits('importItem', 'importUnit'); populateMovementUnits('exportItem', 'exportUnit'); populateProductionFields(); }
