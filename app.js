...
function authErrorMessage(error) { const message = String(error?.message || error || ''); if (/email.*disabled|email.*not.*enabled/i.test(message)) return 'تسجيل الحسابات متوقف حاليًا. من Supabase افتح Authentication ثم Providers ثم Email، فعّل Email، وأوقف Confirm email فقط.'; if (/already registered|already exists/i.test(message)) return 'اسم المستخدم مستخدم من قبل. اختر اسمًا آخر أو انتقل إلى تسجيل الدخول.'; if (/invalid login credentials/i.test(message)) return 'اسم المستخدم أو كلمة المرور غير صحيحة.'; return 'تعذر إتمام العملية. تحقق من البيانات وإعدادات الحساب ثم حاول مرة أخرى.'; }
