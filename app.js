...
function printReport(type) {
  ...
  const rows = type === 'inventory' ? Object.values(calculateInventory().items || {}).map((item) => `<tr><td>${esc(item.name)}</td><td>${esc(item.category)}</td><td>${num(item.qty).toFixed(2)}</td><td>${esc(unitLabel(item.unitCategory, item.baseUnit))}</td><td>${money(item.avgCost)}</td><td>${money(item.value)}</td></tr>`).join('') : type === 'imports' ? state.imports.map((item) => `<tr><td>${esc(item.date)}</td><td>${esc(item.supplier)}</td><td>${esc(product(item.itemId)?.name || '-')}</td><td>${num(item.qty).toFixed(2)}</td><td>${money(landedCost(item))}</td></tr>`).join('') : type === 'exports' ? state.exports.map((item) => `<tr><td>${esc(item.date)}</td><td>${esc(item.customer)}</td><td>${esc(product(item.itemId)?.name || '-')}</td><td>${num(item.qty).toFixed(2)}</td><td>${money(item.revenue)}</td></tr>`).join('') : state.expenses.map((item) => `<tr><td>${esc(item.date)}</td><td>${esc(item.type)}</td><td>${esc(item.description)}</td><td>${money(item.amount)}</td></tr>`).join('');
  const configs = { inventory: ['كشف المخزون', ['الصنف','الفئة','الرصيد','الوحدة','متوسط التكلفة','القيمة']], imports: ['سجل الواردات', ['التاريخ','المورد','الصنف','الكمية الأساسية','التكلفة الواصلة']], exports: ['سجل المبيعات والصادرات', ['التاريخ','العميل','الصنف','الكمية الأساسية','الإيراد']], expenses: ['سجل المصروفات', ['التاريخ','النوع','الوصف','المبلغ']] }; const [title, headers] = configs[type]; printDocument(title, `REPORT-${today()}`, `<table><thead><tr>${headers.map((header) => `<th>${header}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table>`);
}
function printDocument(title, number, body) { const win = window.open('', '_blank', 'noopener,noreferrer'); if (!win) return notify('اسمح بالنوافذ المنبثقة لطباعة المستند', true); win.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>${esc(title)} - ${esc(number)}</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,Tahoma,sans-serif;color:#172433;margin:0;font-size:13px}.sheet{max-width:900px;margin:auto}.header{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #173b64;padding-bottom:16px;margin-bottom:18px}.brand h1{margin:0 0 5px;font-size:24px;color:#173b64}.brand p{margin:0;color:#60758c}.doc{text-align:left;direction:ltr}.doc h2{margin:0 0 7px;color:#173b64;font-size:19px}.doc div{color:#52677c;margin:3px 0}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:9px;background:#f3f7fb;border:1px solid #d8e2ec;border-radius:7px;padding:12px;margin-bottom:18px}.info-grid b{color:#173b64}table{width:100%;border-collapse:collapse;margin:12px 0 18px}th{background:#173b64;color:white;font-weight:700}th,td{border:1px solid #ccd8e3;padding:9px;text-align:right}tbody tr:nth-child(even){background:#f7fafc}.totals{margin-right:auto;width:310px;border-top:2px solid #173b64}.totals div{display:flex;justify-content:space-between;padding:8px;border-bottom:1px solid #d8e2ec}.totals div:last-child{font-size:15px;color:#173b64}.note{margin-top:28px;padding:12px;background:#fff8e8;border-right:4px solid #d97706;color:#5b4a22}.footer{border-top:1px solid #d8e2ec;margin-top:42px;padding-top:12px;color:#718397;display:flex;justify-content:space-between}.signature{margin-top:48px;display:flex;justify-content:space-between}.signature span{border-top:1px solid #9aaabd;padding-top:8px;width:180px;text-align:center}@media print{.no-print{display:none}} </style></head><body><main class="sheet"><header class="header"><div class="brand"><h1>${esc(state.settings.factoryName)}</h1><p>الجمهورية الليبية — مستند تجاري</p><p>هاتف المصنع: ____________</p></div><div class="doc"><h2>${esc(title)}</h2><div>رقم المستند: ${esc(number)}</div><div>تاريخ الإصدار: ${esc(today())}</div></div></header>${body}<div class="signature"><span>توقيع المسؤول</span><span>توقيع العميل / المورد</span></div><footer class="footer"><span>${esc(state.settings.factoryName)}</span><span>العملة: ${esc(state.settings.currency)} — مستند صادر من النظام</span></footer></main><script>window.onload=()=>{window.focus();window.print()}<\/script></body></html>`); win.document.close(); }
